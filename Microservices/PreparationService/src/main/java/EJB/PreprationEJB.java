...
public class PreprationEJB implements PreprationEJBLocal {
    ...
    try{
            List<OrderMaster> orders= (List<OrderMaster>) em.createQuery("SELECT o FROM OrderMaster o Where o.userId.id =:id and o.orderStatus !=:status").setParameter("id", customerid).setParameter("status", OrderStatus.DELIVERED.toString()).getResultList();
