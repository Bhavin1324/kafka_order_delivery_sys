...
import com.mycompany.utils.GenerateToken;
...
//    EntityManager em = emf.createEntityManager();
        ...
        try {
            ...
            if (pbk.verify(data.getString("password").toCharArray(), user.getPassword())) {
                user_details = Json.createObjectBuilder()
                        ...
                        .add("token", GenerateToken.generateJWT(role, Constants.ONE_DAY_EXP_TOKEN))
                        .build();
            }
            return user_details;
    ...
    public Boolean updateUserCredits(String userid, Double credits) {
        Users u = em.find(Users.class, credits);
        if(em.contains(u))
        {
