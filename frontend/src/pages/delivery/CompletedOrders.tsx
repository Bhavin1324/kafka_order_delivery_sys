...
const CompletedOrders = () => {
  ...
  const DisplayHook = useFetch(import.meta.env.VITE_ORDER_SERVICE_URI+ ApiEndpoints.GET_ORDERS_FOR_DELIVERY+dpid+"/"+"DELIVERED" ,"GET")
  useEffect(() => {
      ...
      {orders.map((order) => (
        <li key={order.id} className="rounded-md shadow-md p-4 flex  justify-between items-center cursor-pointer hover:bg-gray-100" onClick={() => handleOrderClick(order)}>
          <div className="mr-4 flex flex-col flex-wrap">
            ...
            <span className="text-base font-semibold text-gray-500">{order.phoneNo} <br/>{order.address}</span>
          </div>
          <div className=" flex flex-col flex-wrap w-1/3 text-right">
            <span className="text-xl font-semibold">₹{order.payable_amount}</span>
            <span className="text-base text-gray-500 ml-2">{order.items.length} items</span>
          </div>
        </li>
      ))}
    </div>
