...
const PreparedOrders = () => {
  ...
  return (
    <div className="mt-4 overflow-x-auto container">
     ...
     {list.length!= 0 &&  !isNotFound &&<table className="table  table-hover" >
      <thead>
        <tr>
      ...
      <tbody>
        {list.map((order) => (
          ...
          <tr key={order.id} className={ order.status=="DELIVERED"? "table-success":(order.status=="CANCELLED"?"table-danger":(order.status == "IN_TRANSIT"?"":"")) } >
            <td>{order.id}</td>
            <td>{order.name}</td>
            <td>{order.totalAmount}</td>
            <td className={ order.status=="DELIVERED"?  " text-success  font-bold":(order.status=="CANCELLED"?" text-danger font-bold":(order.status == "IN_TRANSIT"?"font-bold text-info":""))}>{order.status}</td>
